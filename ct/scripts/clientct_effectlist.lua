...
function onInit()
	...
	DB.addHandler(DB.getPath(node, "effects"), "onChildAdded", onEffectsUpdated);
...
function onClose()
	local node = window.getDatabaseNode();
	DB.removeHandler(DB.getPath(node, "effects"), "onChildAdded", onEffectsUpdated);
	DB.removeHandler(DB.getPath(node, "effects.*"), "onChildUpdate", onEffectsUpdated);
	DB.removeHandler(DB.getPath(node, "effects"), "onChildDeleted", onEffectsUpdated);
