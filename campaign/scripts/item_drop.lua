...
function onDrop(x, y, draginfo)
	...
	-- genesys added referencegear
	if not StringManager.contains({"item", "referencearmor", "referenceweapon", "referenceequipment", "referencegear"}, sDropClass) then
		return true;
	end
	...
	if sSourceType == sTargetType and StringManager.contains({ "Weapon", "Armor" }, sSourceType) then
		...
		if ((sSourceAura == "") and (sTargetAura ~= "")) then
			...
			-- Changes for Genesys Needed
			if sSourceType == "Weapon" then
				DB.setValue(nodeTarget, "subtype", "string", DB.getValue(nodeSource, "subtype", ""));
				DB.setValue(nodeTarget, "encumbrance", "string", DB.getValue(nodeSource, "encumbrance", ""));
