...
function onInit()
	...
	ChatManagerGenesys.registerSpecialMessageHandler(SPECIAL_MSGTYPE_UPDATENONOWNEDDB, handleUpdateNonOwnedDB);

	-- Used to create new fields.
	ChatManagerGenesys.registerSpecialMessageHandler(SPECIAL_MSGTYPE_CREATENONOWNEDDB, handleCreateNonOwnedDB);

	-- Used to create a new child record - only works if the child record is named.  Doesn't support id-XXXXX child records.
	ChatManagerGenesys.registerSpecialMessageHandler(SPECIAL_MSGTYPE_CREATECHILDNONOWNEDDB, handleCreateChildNonOwnedDB);

	-- Used to create a new critical inury record
	ChatManagerGenesys.registerSpecialMessageHandler(SPECIAL_MSGTYPE_CRITICALNOWNEDDB, handleCreateCriticalNonOwnedDB);

	-- register some bogus message handlers so CoreRPG.OOBManager does not throw us some invalid OOB types
	-- Need to improve this in the future, find a better solution...
	OOBManager.registerOOBMsgHandler(SPECIAL_MSGTYPE_UPDATENONOWNEDDB, ProcessNothing);
	OOBManager.registerOOBMsgHandler(SPECIAL_MSGTYPE_CREATENONOWNEDDB, ProcessNothing);
	OOBManager.registerOOBMsgHandler(SPECIAL_MSGTYPE_CREATECHILDNONOWNEDDB, ProcessNothing);
	OOBManager.registerOOBMsgHandler(SPECIAL_MSGTYPE_CRITICALNOWNEDDB, ProcessNothing);
...
function updateNonOwnedDB(nodeRecord, dbFieldName, value)
	...
	ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_UPDATENONOWNEDDB, msgparams);
...
function createNonOwnedDB(nodeRecord, dbFieldName, dbFieldType, value)
	...
	ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_CREATENONOWNEDDB, msgparams);
...
function createChildNonOwnedDB(nodeRecord, dbChildName)
	...
	ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_CREATECHILDNONOWNEDDB, msgparams);
...
function createCriticalNonOwnedDB(nodeRecord, name, description, severity)
	...
	ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_CRITICALNOWNEDDB, msgparams);
