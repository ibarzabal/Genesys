...
function performPCPowerAction(draginfo, nodeAction, sSubRoll)
	...
	if rAction then
		if rAction.type == "cast" then
			-- GENESYS
			performActionGenesys(rActor, rAction, nodeAction);
		else
...
function getPowerGroupRecord(rActor, nodePower, bNPCInnate)
	...
	if sActorType == "pc" then
		...
		if nodeTrait then
			...
			if nFixedAtk then
				local nTempMod = ActorManager2.getAbilityBonus(rActor, aPowerGroup.sStat) + ActorManager2.getAbilityBonus(rActor, "prf");
			...
			if nFixedSaveDC then
				local nTempMod = 8 + ActorManager2.getAbilityBonus(rActor, aPowerGroup.sStat) + ActorManager2.getAbilityBonus(rActor, "prf");
...
function evalAction(rActor, nodePower, rAction)
	...
	if (rAction.type == "cast") or (rAction.type == "attack") then
		...
		if (rAction.stat or "") ~= "" then
			rAction.modifier = (rAction.modifier or 0) + ActorManager2.getAbilityBonus(rActor, rAction.stat);
		end
		if (rAction.prof or 0) == 1 then
			rAction.modifier = (rAction.modifier or 0) + ActorManager2.getAbilityBonus(rActor, "prf");
		end
	end
	...
	if (rAction.type == "cast") or (rAction.type == "powersave") then
		...
		if (rAction.savestat or "") ~= "" then
			rAction.savemod = (rAction.savemod or 8) + ActorManager2.getAbilityBonus(rActor, rAction.savestat);
		end
		if (rAction.saveprof or 0) == 1 then
			rAction.savemod = (rAction.savemod or 8) + ActorManager2.getAbilityBonus(rActor, "prf");
		end
	end
		...
		for _,vClause in ipairs(rAction.clauses) do
			if (vClause.stat or "") ~= "" then
				if vClause.stat == "base" then
					...
					if aPowerGroup then
						local nAbilityBonus = ActorManager2.getAbilityBonus(rActor, aPowerGroup.sStat);
					...
					local nAbilityBonus = ActorManager2.getAbilityBonus(rActor, vClause.stat);
					local nMult = vClause.statmult or 1;
					if nAbilityBonus > 0 and nMult ~= 1 then
						nAbilityBonus = math.floor(nMult * nAbilityBonus);
					end
...
function performActionGenesys(rActor, rAction, nodeAction)
	if not rActor or not rAction then
		return false;
	end

	local nodeActionSkill = DB.getValue(nodeAction,"skill","");
	local nodeActionDifficultyMod = DB.getValue(nodeAction,"DifficultyMod",0);
	local nodeActionRange = DB.getValue(nodeAction,"range","");
	-- search skill node
	local nodeSkillList = DB.getChildren(nodeAction,".....skilllist");
	local locatedskillnode = nil;
	for k,v in pairs(nodeSkillList) do
		--Debug.console("Looking at current child: " .. k .. ", skillname = " .. v.getChild("name").getValue());
		if string.lower(v.getChild("name").getValue()) == string.lower(nodeActionSkill) then
	...
	local msgidentity = rActor.Name;
	if locatedskillnode then
		DicePoolManager.addSkillDice(locatedskillnode, dice);
		if table.getn(dice) > 0 then
			if locatedskillnode.getChild("name") then
...
function parsePCPower(nodePower)
	...
	-- Otherwise, parse the power description for actions
		...
		if bConcentration and not bConcEffectFound then
			table.insert(aActions, { type = "effect", sName = sPowerName .. "; (C)", sTargeting="self", nDuration = nDuration, sUnit = sDurationUnits });
		end
