-- GENESYS: original script from Core RPG
-- Several functions updated to work on Genesys
...
function getRequestingNextActor()
	-- Checks if we are requesting Players/GM to select next actor
	for _,v in pairs(getCombatantNodes()) do
		if DB.getValue(v, "ctentry_choose_next", 0) == 1 then
			return v;
		end
	end
	return nil;
...
function requestActivation(nodeEntry, bSkipBell, bisHost)
	...
	if #aEntries > 0 then
		if nodeEntry then
			for i = 1,#aEntries do
				if DB.getValue(aEntries[i], "ctentry_turn_done") == 0 and nodeEntryfriendfoe == DB.getValue(aEntries[i], "friendfoe", "") then
					if aEntries[i] ~= nodeEntry then
						local firstNodeMatchinitresult = DB.getValue(aEntries[i], "initresult");
						local nodeEntryinitresult = DB.getValue(nodeEntry, "initresult");
						DB.setValue(aEntries[i], "initresult", "number", nodeEntryinitresult);
						DB.setValue(nodeEntry, "initresult", "number", firstNodeMatchinitresult);
					end
					break;
				end
			end
  ...
  onTurnStartEvent(nodeEntry);
	-- De-activate all other entries
	for _,v in pairs(getCombatantNodes()) do
		DB.setValue(v, "active", "number", 0);
		DB.setValue(v, "ctentry_choose_next", "number", 0);
	end
		...
		if nodeActive then
			for i = 1,#aEntries do
				if aEntries[i] == nodeActive then
					nIndexActive = i;
					DB.setValue(nodeActive, "ctentry_turn_done", "number", 1);  -- Flag this actor as already done with turn
					break;
				end
			end
		end
...
-- Genesys
function RequestNextActor(nodeNext)
	local nodeActivefriendfoe = DB.getValue(nodeNext, "friendfoe", "");
	-- Show possible next actors
	local aEntries = getSortedCombatantList();
	if #aEntries > 0 then
		if nodeNext then
			for i = 1,#aEntries do
				DB.setValue(aEntries[i], "active", "number", 0);
				if DB.getValue(aEntries[i], "ctentry_turn_done") == 0 and nodeActivefriendfoe == DB.getValue(aEntries[i], "friendfoe", "") then
					DB.setValue(aEntries[i], "ctentry_choose_next", "number", 1);  -- Flag this actor as already done with turn
				end
			end
		end
	end
...
function nextRound(nRounds)
	...
	for _,v in pairs(getCombatantNodes()) do
		DB.setValue(v, "ctentry_turn_done", "number", 0);
	end
