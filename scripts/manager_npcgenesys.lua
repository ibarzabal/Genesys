...
function onDrop(npcnode, x, y, draginfo)
	...
	Debug.console("NPCManagerGenesys.lua:onDrop: Type = " .. draginfo.getType() .. ", node = " .. npcnode.getNodeName());
	...
	if draginfo.isType("shortcut") then
		...
		Debug.console("NPCManagerGenesys.lua:onDrop: Shortcut class = " .. class);
...
function removeChildRecharge(node)
	...
	if node and node.isOwner() then
		...
		if string.find(node.getParent().getNodeName(), "conditions") then
			...
			if durationnode and string.lower(durationnode.getValue()) == "brief" then
...
--	if User.isHost() then
		...
		if npcnode.getChild("npc_category") then
			if string.lower(npcnode.getChild("npc_category").getValue()) == "nemesis" or string.lower(npcnode.getChild("npc_category").getValue()) == "rival" then
				...
				--if DBManagerGenesys.checkForDuplicateName(criticalsnode, criticalnode) then
				--	return false;
				--end
				...
				Comm.throwDice("dice", dice, modifier, description, {npcnodename, msgidentity, gmonly});
