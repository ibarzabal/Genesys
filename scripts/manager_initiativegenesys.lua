...
-- This used to belong to CharacterManager in the old ruleset. I will use it here for now
...
--	ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_PCSELECTED, msgparams)

end



-- Used to trick OOBManager.registerSpecialMessageHandler to do nothing while we use Genesys messager
...
function onInit()
	...
	ChatManagerGenesys.registerSpecialMessageHandler(SPECIAL_MSGTYPE_REFRESHACTORLIST, handleRefreshActorList);
	ChatManagerGenesys.registerSpecialMessageHandler(SPECIAL_MSGTYPE_REFRESHINITSLOTLIST, handleRefreshInitSlotList);
	ChatManagerGenesys.registerSpecialMessageHandler(SPECIAL_MSGTYPE_ADDTOINITSLOT, handleClientAddToInitSlot);
	ChatManagerGenesys.registerSpecialMessageHandler(SPECIAL_MSGTYPE_NEXTACTOR, handleNextActor);

	OOBManager.registerOOBMsgHandler(SPECIAL_MSGTYPE_REFRESHACTORLIST, handleRefreshActorList);
	OOBManager.registerOOBMsgHandler(SPECIAL_MSGTYPE_REFRESHINITSLOTLIST, handleRefreshInitSlotList);
	OOBManager.registerOOBMsgHandler(SPECIAL_MSGTYPE_ADDTOINITSLOT, handleClientAddToInitSlot);
	OOBManager.registerOOBMsgHandler(SPECIAL_MSGTYPE_NEXTACTOR, handleNextActor);
...
function clientAddToInitSlot(actornode)
	...
	ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_ADDTOINITSLOT, msg);
end
...
function refreshActorList()
	ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_REFRESHACTORLIST, {});
end
...
function refreshInitSlotList()
	...
	ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_REFRESHINITSLOTLIST, {});
end
	...
	if activeinitslotdbnode then
		...
		if isOwnedIdentity(DB.findNode(activeinitslotdbnode.getChild("initslot_actornodename").getValue())) then
			...
			--if activeinitslotdbnode.getChild("actor.classname").getValue() == "charsheet" then
				ChatManagerGenesys.sendSpecialMessage(SPECIAL_MSGTYPE_NEXTACTOR, msg);
			--end
		end
	end
end
